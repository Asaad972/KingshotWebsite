...
export async function POST(request: Request, { params }: { params: { slug: string; id: string } }) {
  ...
  // admin_accept_kingdom_application is SECURITY DEFINER and re-verifies
  // the admin token itself, and re-checks the slot isn't already booked
  // atomically at the database level -- that's what actually prevents the
  // double-accept race, not anything in this route handler.
  const { data, error } = await supabase.rpc('admin_accept_kingdom_application', {
  ...
  const status = data?.success ? 200 : data?.reason === 'invalid_token' ? 401 : 409;
