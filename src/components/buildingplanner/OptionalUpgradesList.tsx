...
export default function OptionalUpgradesList({
  ...
  return (
    <div className="flex flex-col gap-2.5">
      <h2 className="text-sm font-semibold text-parchment-300">Optional Upgrades</h2>
      <p className="text-xs text-parchment-500 -mt-1.5">
        Not required to reach this Town Center target, but you can add them to the plan anyway.
      </p>
      <div className="flex flex-col gap-2.5">
        {optionalIds.map((id) => (
          <OptionalRow
  ...
  return (
    <div className="dashboard-card p-3.5 flex items-center gap-3.5">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-parchment-400">Current: {currentLevel ? `Lv.${currentLevel}` : 'None'}</p>
      </div>

      {addedTarget ? (
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-xs font-semibold text-gold-300">Target Lv.{addedTarget}</span>
          <button
            type="button"
            onClick={onRemove}
